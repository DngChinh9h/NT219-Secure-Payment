 ...
 */
async function encryptUserPII({ fullName, address, cccdNumber }) {
  const { plaintext: dataKey, wrapped } = await generateDataKey();
 ...
 */
async function decryptUserPII(userRow) {
  const dataKey = await unwrapDataKey(userRow.wrapped_data_key);
