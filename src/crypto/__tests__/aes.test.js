const crypto = require('crypto');
    ...
    const testKey = crypto.randomBytes(32); // Data key ngẫu nhiên cho test
    ...
    describe('AES Helper', () => {
      ...
      test('2 lần encrypt cùng plaintext → IV khác nhau', () => {
        ...
        expect(e1.iv).not.toBe(e2.iv);            // IV phải khác
        expect(e1.ciphertext).not.toBe(e2.ciphertext); // Ciphertext cũng khác
      });
    
      test('authTag bị sửa → throw Error (tamper detection)', () => {
        const enc = aesEncrypt('sensitive PII data', testKey);
        const tampered = { ...enc, authTag: 'a'.repeat(32) }; // fake authTag
